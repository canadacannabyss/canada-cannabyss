...
const Banner = () => {
  ...
  useEffect(() => {
    if (!_.isEmpty(bannerDetails)) {
      setId(bannerDetails._id);
      setSlug(bannerDetails.slug);
      setBannerName(bannerDetails.bannerName);
      setDescription(bannerDetails.description);

      setSeoTitle(bannerDetails.seo.title);
      setSeoSlug(bannerDetails.seo.slug);
      setSeoDescription(bannerDetails.seo.description);
      if (
        bannerDetails.organization.category === null ||
        bannerDetails.organization.category === undefined
      ) {
        setCategory('');
      } else {
        setCategory(bannerDetails.organization.category.categoryName);
      }
      setTagsArray(bannerDetails.organization.tags);
      setTags(handleTagsArrayToTagString(bannerDetails.organization.tags));
      setFeatured(bannerDetails.featured);
    }
  }, [bannerDetails]);
  ...
  return (
    <>
      <Layout>
        <Head>
          <title>{`${
            bannerName.length && bannerName
          } | Edit - Canada Cannabyss`}</title>
        </Head>
        <Wrapper>
          <ProductListDiv>
            <PromotionsList
          ...
          <Button onClick={onSubmit}>Create Promotion</Button>
        </Wrapper>
      </Layout>
      {loading && (
        <Loading>
